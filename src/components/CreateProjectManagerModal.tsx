...
import {CreateProjectManagerModalProps} from "@/types/ProjectManager";
...
export default function CreateProjectManagerModal({
                                                  ...
                                                  }: CreateProjectManagerModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('http://localhost:8080/projects/project-managers', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token ? `Bearer ${token}` : '',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to add project manager');
      }
      const updatedProject = await response.json();
      setSuccessMessage('Project manager added successfully!');
      setTimeout(() => {
  ...
  return (
      <div className="fixed inset-0 flex items-center justify-center z-50">
        ...
        <div className="relative bg-white p-6 rounded-lg shadow-md w-full max-w-md">
          ...
          <h2 className="text-2xl font-bold text-blue-600 mb-4 text-center">Add Project Manager</h2>
